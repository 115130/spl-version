...
## 22.12 设备配置的生命周期：创建、烧录、轮换、失效

真实设备身份不是一个写死在 `main.c` 的字符串，而是一条可追踪流程：

~~~text
生成每设备配置 → 用演示向量验证 → 安全地烧录/注入
       ↓                                  ↓
仓库只存模板 ← 版本与设备记录 ← 设备启动只输出脱敏标识
       ↓
轮换 / 吊销 / 恢复出厂
~~~
...
| 阶段 | 最低要求 |
|---|---|
| 开发 | 使用演示 Secret 与本地 Broker，不碰真实生产凭据 |
| 构建 | `device_config.example.h` 可提交，真实 `device_config.h` 被忽略 |
| 烧录 | 记录设备 ID、固件版本、配置版本；不要把 Secret 打进串口日志 |
| 运行 | 授权失败有计数和明确错误码，不能无限重试 |
| 吊销/轮换 | 平台侧能禁用丢失设备；固件能识别配置版本变化 |
| 退役 | 清除或失效凭据，并保留审计记录 |

STM32F103 这类教学 MCU 不应被描述为硬件安全根。读保护、封装和生产流程可以降低风险，但不能替代平台权限、最小权限设计和密钥轮换。高价值设备需要按威胁模型评估安全芯片、受保护烧录和服务端策略。
...
{
    printf("[cloud] product=%s device=%s secret_len=%u\r\n",
           c->product_id, c->device_id, (unsigned)c->secret_len);
    /* 绝不打印 c->secret 或完整签名。 */
}
~~~

练习：给配置增加 `config_version`，启动时只打印版本号和设备 ID；用“版本不匹配”模拟一次受控拒绝，而不是继续带着未知凭据连接。

## 22.13 在 F103 上先做可行性决策，再接真实云端

HMAC 与 TLS 都不是“复制一段 C 代码”就自动安全。ZET6 有 64KB SRAM，本书又使用通用 AT 模块；应在选平台前明确由谁做密码学工作、证书如何验证、时间从哪里来。

| 方案 | 适用条件 | 必须留下的证据 |
|---|---|---|
| 模块代管 TLS | 模块文档明确支持目标协议、服务器名/SNI、证书配置和错误码 | 固件版本、证书加载方式、握手/失败日志、峰值 RAM/供电测试 |
| MCU 只做 HMAC | 使用经过审查的库或已验证实现，TLS 由模块完成 | 库版本/许可证、公开测试向量、Flash/RAM 增量、失败处理 |
| 明文教学服务 | 仅限隔离局域网与公开演示数据 | 文档中显式标注“非真实云端安全路径” |
| 自己手写 HMAC/TLS | 不作为本书入门路径 | 需要独立的密码学审查与测试，不能靠连通一次证明正确 |

### 规范化字符串应独立于平台网络代码

先把“平台规则”封装成一个纯字节函数，再把它交给 HMAC 和 MQTT/HTTP 层：
...
/* 只生成平台规定的、可打印和可逐字节比较的 canonical 文本。
   不读取 Secret，不发送网络。 */
int Cloud_BuildCanonical(const CloudAuthInput *in,
                         char *out, size_t out_size);
~~~

PC 脚本、MCU 和平台文档必须对同一 `canonical` 文本一致后，才比较 HMAC。字段排序、百分号编码、换行、UTF-8 和十六进制大小写都属于协议，而不是“显示细节”。

### 时间、烧录与轮换也是身份的一部分

- **首次烧录**：给设备写入唯一 ID、公开标识、Secret 的安全来源/版本；仓库只保留 example 文件。
- **首次联网前**：说明时间戳来自 RTC、受控配网、模块/NTP 还是平台允许的其他机制；时钟未知时不要假装能正确校验证书或签名时限。
- **运行中**：日志只记录设备 ID 的脱敏形式、错误码、配置版本和签名输入长度，绝不记录 Secret。
- **泄露/退役**：平台侧撤销旧凭据，烧录或配置流程产生新版本；旧 Secret 不能继续作为“备用”。

练习：用一个含 UTF-8 字符、空字段和固定时间戳的公开向量，分别在 PC 和 MCU 输出 canonical 字节的十六进制。先证明输入一致，再比较 HMAC。
