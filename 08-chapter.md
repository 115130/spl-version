...
> **本章产出**：从电平、帧格式、时钟到 SPL 初始化，搭建一个能收、能发、能定位错误的 USART1 调试通道；后续所有模块先用它输出证据。
>
> **前置知识**：第 3 章 GPIO、第 5 章时钟、第 6 章中断。
>
> **硬件准备**：3.3V USB-TTL；ZET6 USART1 的 PA9(TX)、PA10(RX) 与 GND。TX/RX 必须交叉，所有设备必须共地。
>
> **通过标准**：终端能稳定看到启动日志，并能向开发板发送一条命令，得到可预测的回应。
...
把 UART 当成后续每章的“观察口”。先完成这三个最小实验：

1. **只发不收**：上电打印固定字符串，确认 PC 端波特率和换行；
2. **只收不发**：从终端发送一个字符，开发板点亮/熄灭 LED；
3. **收发闭环**：输入 `ping`，设备返回 `pong\r\n`，并在接收缓冲区溢出时打印计数。

| 现象 | 优先检查 | 不要先做什么 |
|---|---|---|
| 完全没有输出 | PA9 是否接到 USB-TTL RX、GND 是否相连、程序是否真的运行 | 立刻重写 printf |
| 全是乱码 | 两端波特率、系统时钟、USB-TTL 电平 | 只改终端编码 |
| 偶尔丢字节 | ISR 是否及时取数据、缓冲区是否太小、是否在 ISR printf | 用无限大 delay 掩盖 |
| 收到自己发出的数据 | TX/RX 接线或回环方式 | 误以为模块已经响应 |
| 一开中断就卡死 | IRQHandler 名称、RXNE 标志清除、NVIC | 在 ISR 里做命令解析 |

练习：实现一个 `stats` 命令，输出接收字节数、溢出数和最后一次命令耗时。它会成为后续 AT、Modbus 和网关实验的统一诊断接口。
