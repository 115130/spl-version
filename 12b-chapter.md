...
> **本章产出**：能正确接入半双工 RS485，构造并验证一帧 Modbus RTU 请求；遇到超时或 CRC 错误时能从电气层到协议层逐步定位。
>
> **前置知识**：第 8 章 UART、第 6 章中断；了解二进制帧和 CRC 的基本概念。
>
> **硬件准备**：带收发器的 RS485 模块、两端设备的共地与明确 A/B 标记；总线较长或节点较多时再按拓扑添加终端和偏置。
...
## 12.5.4 Modbus RTU —— 485 上的"普通话"（重点）
...
## 12.5.5 完整代码：读取 RS485 SHT30 温湿度
...
## 12.5.8 RS485/Modbus 的验收与练习

从最小链路开始，不要一开始就接长线、多节点和复杂寄存器表：

1. 先用 USB-RS485 或第二块设备确认 A/B 和 GND；
2. UART 打印“发出帧”和“收到帧”的十六进制；
3. 从固定的一条 `0x03` 读寄存器请求开始，检查地址、长度和 CRC；
4. 再测试超时、错误 CRC、设备离线和连续轮询。

| 现象 | 优先检查 |
|---|---|
| 完全无响应 | A/B 是否接反、DE/RE 方向、波特率、从站地址、共地 |
| 能收但 CRC 全错 | 帧边界、字节顺序、CRC 算法、串口参数 |
| 只收自己的数据 | 发送后没有切回接收，或收发器回显行为未处理 |
| 长线偶发错误 | 终端、偏置、拓扑、速率和地电位差 |

练习：实现一个“每秒轮询一次，连续三次失败才标记离线”的状态机，并通过 UART 输出成功、超时和 CRC 错误各自的计数。
