...
> **本章产出**：能计算并测出定时器周期、PWM 频率和占空比；能用输入捕获把外部脉冲变成可读数值。
>
> **前置知识**：第 5 章时钟、第 6 章中断、第 3 章 GPIO。
>
> **硬件准备**：板载 LED 可完成 PWM/呼吸灯；若测量真实频率，使用逻辑分析仪或示波器，并避免把未知电压接入输入捕获引脚。
...
每一个定时器实验都先写出公式，再接仪器或 LED 验证：

`f = TIMCLK / (PSC + 1) / (ARR + 1)`

| 现象 | 优先检查 |
|---|---|
| PWM 频率不对 | TIMCLK 是否受 APB 分频和倍频规则影响；PSC/ARR 是否按公式计算 |
| 占空比反了或不变 | 通道号、GPIO 复用输出、CCR 范围与 ARR 的关系 |
| 中断不进 | 更新中断使能、NVIC、IRQHandler、pending 清除 |
| 输入捕获数值跳动 | 输入电平、边沿选择、计数溢出和信号噪声 |

练习：
1. 将呼吸灯占空比限制在 5%–95%，解释为什么不建议直接从 0% 跳到 100%；
2. 用输入捕获测一个已知频率的方波或按键周期；
3. 记录一次实际测量值，并解释它与计算值的误差来源。
